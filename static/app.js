class RealtimeDemo {
    constructor() {
        ...
        this.deltaBuffers = new Map(); // item_id -> accumulated delta text
    ...
    sendTextMessage() {
        const text = (this.textInput && this.textInput.value ? this.textInput.value : '').trim();
        if (!text) return;
    ...
    handleRealtimeEvent(event) {
        ...
        // Handle model-level events (both wrapped and direct forms).
        try {
            this.handleModelEvent(event);
        } catch (err) {
            console.error('[handleModelEvent] error:', err, 'event:', event);
        }
    }

    getModelEventType(event) {
        if (!event || typeof event !== 'object') return null;
        // Support legacy raw_model_event wrapper from older server versions
        if (event.type === 'raw_model_event' && event.raw_model_event) {
            return typeof event.raw_model_event.type === 'string'
                ? event.raw_model_event.type
                : null;
        }
    ...
    handleModelEvent(event) {
        const modelType = this.getModelEventType(event);
        if (!modelType) return;
        ...
        if (modelType === 'conversation.item.input_audio_transcription.completed') {
            ...
            if (transcription && itemId) {
                this.updateMessageTranscription(itemId, transcription, 'user');
            }
            return;
        }

        if (modelType === 'conversation.item.input_audio_transcription.delta') {
            const delta = event.delta;
            if (delta && itemId) this.appendTextDelta(itemId, delta, 'user');
            return;
        }

        if (modelType === 'conversation.item.input_audio_transcription.failed') {
            const msg = event.error || 'input audio transcription failed';
            this.addMessage('assistant', `[transcription failed] ${msg}`);
            return;
        }

        if (modelType === 'response.text.delta' || modelType === 'response.output_text.delta') {
            const delta = event.text_delta || event.delta;
            if (delta && itemId) this.appendTextDelta(itemId, delta, 'assistant');
            return;
        }

        if (
            modelType === 'response.audio_transcript.delta' ||
            modelType === 'response.output_audio_transcript.delta'
        ) {
            const delta = event.audio_transcript_delta || event.delta;
            if (delta && itemId) this.appendTextDelta(itemId, delta, 'assistant');
            return;
        }

        if (modelType === 'response.text.done' || modelType === 'response.output_text.done') {
        ...
        if (
            modelType === 'response.audio_transcript.done' ||
            modelType === 'response.output_audio_transcript.done'
        ) {
    ...
    updateMessageTranscription(itemId, transcription, role) {
        this.applyMessageText(itemId, transcription, role);
    }

    appendTextDelta(itemId, delta, role) {
        // 累积delta文本
        let accumulated = this.deltaBuffers.get(itemId) || '';
        accumulated += delta;
        this.deltaBuffers.set(itemId, accumulated);
        this.applyMessageText(itemId, accumulated, role);
    }

    applyMessageText(itemId, text, role) {
        if (!itemId || !text) return;
        let node = this.messageNodes.get(itemId);
        if (!node) {
            node = this.addMessage(role, text);
            if (node) {
    ...
    updateLastMessageFromHistory(history) {
        ...
        if (Array.isArray(last.content)) {
            for (const part of last.content) {
                ...
                else if (part.type === 'output_audio' && part.transcript) text += part.transcript;
            }
        }
    ...
    addMessageFromItem(item) {
        try {
            ...
            // Deduplicate: skip items we've already rendered
            if (item.item_id && this.seenItemIds.has(item.item_id)) return;
            ...
            if (Array.isArray(item.content)) {
                for (const contentPart of item.content) {
                    ...
                    if (contentPart.type === 'text' && contentPart.text) {
                        ...
                        content += contentPart.transcript;
                    } else if (contentPart.type === 'output_text' && contentPart.text) {
                        content += contentPart.text;
                    } else if (contentPart.type === 'output_audio' && contentPart.transcript) {
                        content += contentPart.transcript;
    ...
    addRawEvent(event) {
        ...
        const eventTitle = this.getModelEventType(event) || event.type || 'unknown';
